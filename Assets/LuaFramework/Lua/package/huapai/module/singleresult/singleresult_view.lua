...
function SingleResultView:initialize(...)
	...
	self.Bottom = Manager.FindObject(self.root, "Bottom")
	...
	self.R1 = Manager.FindObject(self.root, "Bg/Title1/R1")
	self.R2 = Manager.FindObject(self.root, "Bg/Title1/R2")
	self.R3 = Manager.FindObject(self.root, "Bg/Title1/R3")
    ...
    for i = 1, 3 do
        ...
        self.personInfos[i].Image = Manager.GetImage(go, "Mask/Image")
		...
		self.personInfos[i].Fang = Manager.FindObject(go, "Fang")
		self.personInfos[i].Zhuang = Manager.FindObject(go, "Zhuang")

		self.personInfos[i].HuShu = Manager.GetText(go, "HuShu")
		self.personInfos[i].QiangShu = Manager.GetText(go, "QiangShu")

		
		self.personInfos[i].HuImg = Manager.FindObject(go, "HuImg")


		self.personInfos[i].Hu = Manager.FindObject(go, "Hu")

		local huGo = self.personInfos[i].Hu
		self.personInfos[i].HuTable = {}
		for l=1,9 do
			local goZ = huGo.transform:GetChild(l - 1).gameObject
			self.personInfos[i].HuTable[goZ.name] = goZ
		end
		...
		self.personInfos[i].XiaZhangHolderGrid = XiaZhangHolder:GetComponent("GridLayoutGroup")
		print(self.personInfos[i].XiaZhangHolderGrid.name)
		for j=1,10 do
