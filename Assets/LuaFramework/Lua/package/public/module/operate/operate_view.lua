...
function OperateView:updateRecommendGameView()
    ...
    local function set_game_info(operateGame, index, isLocal)
        --先处理是否重复的
    ...
    if usedList ~= nil then
        for i = 1, #usedList do
            ...
            if not set_game_info({province = province.appName, gameId = usedList[i].gameId, type = -1}, curIndex, true) then
                curIndex = curIndex - 1
            end
    ...
    for _, operateGame in ipairs(self.operateList.games) do
        curIndex = curIndex + 1
        if curIndex > GameItemCount then break end
        if  not set_game_info(operateGame, curIndex) then curIndex = curIndex - 1 end
    end
