...
function Mj3dDun:refreshDuns(Dun,TotalCount,DunStart,ZhuangJiaSeat,leftDunCount)
    ...
    if startIndex  % 2 ~= 0 then
        ...
        if (lastMj3d.pai ~= -1 and self.exchangeMj3d) then
            print('77777777777777777777')
            lastMj3d:setMj3Active(false)
            Mj3dHelper:clearMj3d(lastMj3d)
            lastMj3d = self.exchangeMj3d
            lastMj3d:setMj3Active(true)
            self.dunMj3ds[index] = lastMj3d
        end
        local pos = lastMj3d.transform.localPosition
        pos.y = pos.y - dunUpMjYOffset
        lastMj3d.transform.localPosition = pos
    end

    if ((0 == self.view.gameState.Dice1 or self.view.Diceed) and self.view.ConfigData.isEnShiMJ and Dun[leftCount] ~= 0) then
        local anchorMj = Mj3dDun.dunMj3ds[startIndex + 2]
        if (anchorMj.pai == -1) then
            local disMj3d = Mj3d:Create(Dun[leftCount],anchorMj.transform.parent)
            ...
            self.dunMj3ds[startIndex + 2] = disMj3d
            anchorMj:setMj3Active(false)
            self.exchangeMj3d = anchorMj
        end
    end
end
