...
function TableBaseModule:on_update_per_secondOn()
    ...
    if (not audioMusic.isPlaying) then
        local bgMusic1 = "战斗背景音乐"
        local bgMusic2 = "战斗背景音乐1"
        if ((not audioMusic.clip) or audioMusic.clip.name ~= bgMusic2) then
            ModuleCache.SoundManager.play_music("huapai", "huapai/sound/" .. bgMusic2 .. ".bytes", bgMusic2)
        else
            ModuleCache.SoundManager.play_music("huapai", "huapai/sound/" .. bgMusic1 .. ".bytes", bgMusic1)
        end
