...
function TableModule:play_game_state()
    ...
    if data.result ~= 0 then
        ...
        if self:isInZhunBeiIng() and data.result == 1 then
            ...
            Manager.SetActive(self.view.QiZhangObj.gameObject, false)
...
function TableModule:play(data)
    ...
    if not self:has_actionWhat(13) and data.result == 0 then
    ...
    -- 可胡 可过  可出要等用户点击按钮后才显示
    if data.ke_hu and data.ke_chu ~= 0 and data.ke_guo then
        HandCardView:set_out_card_enable(false)

        -- 等待 玩家按下按钮   如果按下的是过 按钮    则 把可出显示出来  isGuoShow
        self:start_lua_coroutine(function()
            if CardCtrlView:isGuoShow() then
                for i=1,1000 do
                    coroutine.wait(0.01)
                    if not CardCtrlView:isGuoShow() then
                        HandCardView:set_out_card_enable(true)
                        return
                    end
                end
            end
...
function TableModule:refresh_game_statePlayer()
...
function TableModule:refresh_paiju(data,valueNotIn)
    ...
    for i = 1, #data.player do
        ...
        self.playersView[localSeatID]:show_QiHu(data.player[i].is_qi_hu)
