    ...
    self:start_lua_coroutine(function()
        ...
        if #intentData.ma_pai > 0 then
            ...
            for i = 1, #intentData.ma_pai do
                ...
                self:XingPaiDongHua(intentData.ma_pai,count,i)
        ...
        if ruleInfo.baseScore then
            ...
            if not curTableData.RoundCount then
                return
            end
...
function SingleResultModule:show_fanxing()
    ...
    Manager.SetActive(self.view.FanXingDongMenGo.gameObject, false)
...
function SingleResultModule:XingPaiDongHua(ma_pai,zhi,index)
    self.view.FanXingDongHuaR.gameObject:SetActive(true)
    
    
    self.view.FanXingDongHuaRImgGOTable[1].gameObject:SetActive(true)
    TableUtilPaoHuZi.set_card(self.view.FanXingDongHuaRImgGOTable[1], 0, nil, 'ZiPai_CurPutCards')
    ...
    for i = 1, 25 do
        ...
        self.view.FanXingDongHuaR.transform.localPosition = UnityEngine.Vector3.New(0, 400 - i * 18, 0)
    end

    for i=1,#self.view.FanXingDongHuaRImgGOTable do
        self.view.FanXingDongHuaRImgGOTable[i].gameObject:SetActive(false)
    end
    
    --self.view.FanXingDongHuaRImgGOTable[index].gameObject:SetActive(true)
    --TableUtilPaoHuZi.set_card(self.view.FanXingDongHuaRImgGOTable[index], ma_pai[index].pai, nil, 'ZiPai_CurPutCards')

    
    Manager.SetActive(self.view.FanXingDongMenGo.gameObject, true)

    for i=1,#self.view.FanXingDongMen do
        self.view.FanXingDongMen[i].gameObject:SetActive(false)
    end

    for i=1,index do
        self.view.FanXingDongMen[i].gameObject:SetActive(true)
        TableUtilPaoHuZi.set_card(self.view.FanXingDongMen[i], ma_pai[i].pai, nil, 'ZiPai_CurPutCards')
    end
