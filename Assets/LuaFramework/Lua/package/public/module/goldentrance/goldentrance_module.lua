...
function GoldEntranceModule:on_click(obj, arg)
    ...
    elseif obj.transform.parent.name == "RoomListContent" then
        ...
        if pd then
            if self.modelData.roleData.gold >= pd.minJoinCoin and self.modelData.roleData.gold <= pd.maxJoinCoin then
                ...
                TableManager:start_enter_gold_matching(self.modelData.roleData.userID, self.modelData.roleData.password, pd.id, nil, nil,nil)
            else
    ...
    elseif obj.transform.parent.gameObject == self.view.wanfalistObj then
        ...
        self:get_data_list()
    elseif objName == "RecordBtn" then --战绩
        --self:getHistoryList()
...
function GoldEntranceModule:get_data_list()
    ...
    print("开始获指定玩法法金币场数据 : "..tostring( self.gameType))
    local requestData = {
    ...
    self:http_get(requestData, function(wwwData)
        ...
        if (retData.success) then
            if type(retData.data ) == "table" then
                ...
                TableManager:start_enter_gold_matching(self.modelData.roleData.userID, self.modelData.roleData.password, data.goldId, nil, nil)
            else
                if init then
...
function GoldEntranceModule:getUserNewMessage()
    ...
    self:http_get(requestData, function(wwwOperation)
        ...
        self:get_userinfo(self.modelData.roleData.userID)
        print(error.error)
    end , function(cacheDataText)
