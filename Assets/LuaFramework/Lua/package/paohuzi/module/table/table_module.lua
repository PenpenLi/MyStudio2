...
function TableModule:set_wanfa()
    ...
    elseif AppData.Game_Name == "XXZP"  then
        ...
        if AppData.App_Name == 'DHHNQP' then
            self.view.txtWanFa.text ="       15胡起胡 "..ruleInfo.roundCount.."胡结算 "
        end
...
function TableModule:refresh_round(round)
    ...
    local jushu = curTableData.RoundCount or DataPaoHuZi.Msg_Table_GameStateNTF.RealyRound

    self.view.txtJushu.text = string.format("第%d/%d局", round, jushu)
    if (AppData.Game_Name == 'XXZP' or AppData.Game_Name == 'LDZP') then
        if DataPaoHuZi.Msg_Table_GameStateNTF then
            self.view.txtJushu.text = string.format("        第%d局", jushu)
        else
...
function TableModule:invite_friend(shareToClipboard)
    ...
    if AppData.Game_Name == 'GLZP' or AppData.Game_Name == 'DYZP' then
        shareData.ruleName = str .. (curTableData.RoundCount or 1) .. '局 ' .. self.view.txtWanFaShow.text
    end
