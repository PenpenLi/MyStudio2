...
function TableModule:initialize(...)
    ...
    self.view.ButtonGPS1.onClick:AddListener(
        function()
    ...
    if not num or type(num) ~= "number" or num == 0 then
        ...
        if num == 0 then
            ...
            for i=1,#self.view.PaiBeiMenChildren do
                self.view.PaiBeiMenChildren[i].gameObject:SetActive(false)
            end
            self.view.PaiBeiMenChildren[6].gameObject:SetActive(true)
        else
            Manager.SetActive(self.view.remainderCardObj, false)
        end
...
function TableModule:invite_friend(shareToClipboard)
   ...
   -- local wanfaName, ruleName = TableUtilPaoHuZi.get_rule_name(self.wanfaTable);

    shareData.ruleName = str .. self.ruleName
