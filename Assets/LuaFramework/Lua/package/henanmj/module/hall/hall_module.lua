    ...
    self:subscibe_module_event("hall", "Event_Hall_GetActivityList", function(eventHead, eventData)
        if self.modelData.hallData.activityList == nil then
            self.view.buttonFreeDiamond.gameObject:SetActive(false);
             ...
             -- 更新奖励视图
            self.hallView:updateRewardView(self.modelData.hallData.activityList, "share");
        end
    end )
...
-- 获取当前游戏是否限免
function HallModule:get_freeByGameId()
    local requestData = {
    ...
    self:http_get(requestData, function(wwwData)
        ...
        if retData.ret == 0 then
            ...
            if retData.data.free and self.modelData.curFree_gameName ~= gameName then
                ModuleCache.ModuleManager.show_public_module("curgamefree", retData.data);
            end
