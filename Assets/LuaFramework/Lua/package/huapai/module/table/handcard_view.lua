    ...
    for i = 1, LINE_COUNT do
        for j = 1, ROW_COUNT do
            if self.cardds[i][j].id ~= DEFAULT_CARD_VALUE then
                ...
                local poscao = self.grid[i][j].position
                ...
                if isFaPai then
                    ...
                    local ya = self.cardds[i][j].obj.transform.position.y
                    self.module:start_lua_coroutine(function ()
                        coroutine.wait(0.2)
                        local posc = poscao
                        posc.y = ya
                        self.cardds[i][j].obj.transform:DOMove(posc, 0.7)
                    end)
            ...
            if pai.isJiang == true and pai.obj then
                ...
                for j = i, #cardds_i do
                    ...
                    if paia.obj then
                        ...
                        weizhi.y = weizhi.y - 69.7 / pai.isJiangNum
