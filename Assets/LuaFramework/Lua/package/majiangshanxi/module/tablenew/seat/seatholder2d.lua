...
---播放多个操作的特效
function SeatHolder2D:play_multi_action_tx(actionArr)
    if(#actionArr == 0) then
        return
    end
    local count = #actionArr
    if (count > 3) then
        count = 3
    end
    for i = 1, count do
        local action = actionArr[i]
        local newChuTX = TableUtil.poor("ChuTX", self.chuTXPos, self:get_aciton_pos(i, count), self.view.poorObjs, self.view.clones)
        local chuimage = GetComponentWithPath(newChuTX, "Animator/WenBen", ComponentTypeName.Image)
        local imageSprite = GetComponentWithPath(newChuTX, "", "SpriteHolder")
        local chuimage1 = GetComponentWithPath(newChuTX, "Animator/GaoLiang", ComponentTypeName.Image)
        local getImage = imageSprite:FindSpriteByName(action .. "")
...
function SeatHolder2D:get_aciton_pos(index, count)
    local hSpace = 340
    local vSpace = 130
    if (count == 1) then
        return Vector3.zero
    elseif self.seatId == 1 or self.seatId == 3 then
        if (count == 2) then
            return Vector3(hSpace*(index - 1) - hSpace/2, 0, 0)
        elseif (count == 3) then
            return Vector3(hSpace*(index - 1) - hSpace, 0, 0)
        end
    elseif self.seatId == 2 or self.seatId == 4 then
        if (count == 2) then
            return Vector3(0, vSpace/2 - vSpace*(index - 1), 0)
        elseif (count == 3) then
            return Vector3(0, vSpace - vSpace*(index - 1), 0)
        end
    end
end
