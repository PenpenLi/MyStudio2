...
function PlayerView:bind_seat()
    if self.view then
        ...
        self.seat = {
            ...
            QiHu = Manager.FindObject(root, "Info/QiHu"),
...
function PlayerView:refresh_game_state(data, actionID, stay)
    ...
    if data then
        ...
        elseif actionID == 11 then
            ...
            if not self:shifouHasXiaoHu() then
                SoundManager:play_action(actionID)
            else
                SoundManager:play_action(111)
            end

            local shibaiFalg = self:shifouyingle()

            if shibaiFalg > 0 then
                -- 播放胜利音乐
                SoundManager:play_nameroot("胜利")
                ModuleCache.SoundManager.audioMusic.mute = true
                coroutine.wait(2.5)
                ModuleCache.SoundManager.audioMusic.mute = false
            elseif shibaiFalg == 0 then
                
            else
                -- 失败音乐
                SoundManager:play_nameroot("失败")
                ModuleCache.SoundManager.audioMusic.mute = true
                coroutine.wait(2.5)
                ModuleCache.SoundManager.audioMusic.mute = false
            end
        ...
        if DataHuaPai.Msg_Table_GameStateNTFNew.result == 0 then
...
-- 是否有笑胡
function PlayerView:shifouHasXiaoHu()
    local data = DataHuaPai.Msg_Table_GameStateNTFNew
    for i = 1, #data.player do
        local dataD = data.player[i]
        for k=1,#dataD.HuFa do
            local name = dataD.HuFa[k].name
            if (string.find(name, '笑胡') ~= nil) then
                return true
            end
        end
    end
...
-- 是否有笑胡
function PlayerView:shifouyingle()
    local winSeat = 0
    local round_score = 0
    local intentData = DataHuaPai.Msg_Table_GameStateNTFNew
    for i = 1, #intentData.player do
        ...
        if localSeatID == 1 then
            round_score = intentData.player[i].round_score
        end
...
function PlayerView:show_QiHu(b)
    self.seat.QiHu:SetActive(b)
end
...
function PlayerView:show_chuzhang(value, playAnim)
    ...
    if self.module.firstGameState or not playAnim then
        ...
        DataHuaPai.chuzhangPosTranTo = self.holder.chuzhang.transform
        if not isPaiDui and self.localSeatID == 1 then
            DataHuaPai.chuzhangPosTranTo = self.holder.chuzhang1.transform
        end
...
function PlayerView:add_qizhang(value, playAnim, stay)
    ...
    if self.IsQiZhangFlag then
        self.IsQiZhangFlag = nil
        return
    end
    ...
    if self.module.firstGameState or not playAnim then
        ...
        if self.dataStateoyl and self.dataStateoyl.chu_pai == 2 and self.localSeatID == 1 then
            self.chuzhang.transform.position = self.holder.chuzhang1.transform.position
        end

        if DataHuaPai.chuzhangPosTranTo then
            self.chuzhang.transform.position = DataHuaPai.chuzhangPosTranTo.transform.position
        end
...
function PlayerView:refresh_xiazhang(data, playAnim, actionID)
    ...
    if actionID == 111 and self.seatIndex == 1 then
        for i = #data, #self.xiazhangList + 1, -1 do
            table.insert(xia_zhang, self:add_xiazhang(data[i],0.5))
        end
    else
        ...
        if count > 0 then
            table.insert(xia_zhang, self:add_xiazhang(data[count],0.5))
        end
    ...
    if self.module.firstGameState or not playAnim then
        ...
        coroutine.wait(0.25)
        self.holder.pinzhang.transform:DOScale(Vector3.New(0.6, 0.3, 0.5), 0.17)
...
---添加下张
---@param cards table
function PlayerView:add_xiazhang(cards,timeBian)
    if not self.xiazhangList then
        self.xiazhangList = {}
    end
    ...
    for i = 1, 6 do
        ...
        if cards.pai[count - i] then
            ...
            TableUtilPaoHuZi.set_card(obj, cards.pai[count -i].pai, nil, "ZiPai_PlayCards")


            
            local paiZhi = cards.pai[count -i].pai
            self.module:start_lua_coroutine(
                function()
                    if timeBian and timeBian > 0 then
                        coroutine.wait(timeBian)
                        if self.IsDestroyPinZhang and paiZhi > 0 then
                            --TableUtilPaoHuZi.set_card(pinObj, paiZhi, nil, "ZiPai_PlayCards")
                        end
                    end
                end
            )
...
function PlayerView:show_chutx(actionID)
    ...
    if actionID == 2 and self.dataStateoyl.chu_pai == 1 then
        actionID = 21
    end
