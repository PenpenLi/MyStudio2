...
function TableBiJiModule:on_destroy()
    ...
    if (self.showResultViewSmartTimer_id) then
        ModuleCache.CSmartTimer.Kill(self.showResultViewSmartTimer_id)
...
function TableBiJiModule:on_model_event_bind()
    ...
    self:subscibe_model_event("Event_Get_Poker", function(eventHead, eventData)
        local beginEvent_Get_Poker = false
        self:subscibe_time_event(2, false, 0):OnComplete(function(t)
            if not beginEvent_Get_Poker then
                ...
                ModuleCache.Log.report_exception("比鸡客户端Event_Get_Poker出错",  text, "")
        ...
        if #eventData.Pokers < 9 then
            ModuleCache.Log.report_exception("比鸡发牌错误",  self.modelData.roleData.userID .. ": " .. ModuleCache.Log.print_pbc_table(eventData), "")
            ModuleCache.GameManager.logout()
            return
        end
        ...
        self.Msg_Get_Pokers = true
        beginEvent_Get_Poker = true;
        self:check_activity_is_open()
    ...
    self:subscibe_model_event("Event_Table_Enter_Room", function(eventHead, eventData)
        if (tostring(eventData.err_no) == "0") then
            ...
            if ((tonumber(eventData.isJoinAfterStart) == 1) and tonumber(eventData.roomInfo.roomStatus) == 1) then --中途加入房间的
                self.Msg_Get_Pokers = true
            end
    ...
    self:subscibe_model_event("Event_Table_Reconnect", function(eventHead, eventData)
        ...
        if (tonumber(eventData.reconnectStatus) == 2) or (tonumber(eventData.reconnectStatus) == 3) then
            self.Msg_Get_Pokers = true
        end
    end )
    ...
    self:subscibe_model_event("Event_Table_Comfirm", function(eventHead, eventData)
        ...
        if not self.Msg_Get_Pokers then
            local text = self.modelData.roleData.userID .. "|" .. self.modelData.curTableData.roomInfo.roomNum .. "|" .. self.modelData.curTableData.roomInfo.curRoundNum
            ModuleCache.Log.report_exception("比鸡可能没有收到手牌包",  text, "")
            ModuleCache.GameManager.logout()
            return
        end
    end )
...
function TableBiJiModule:getSeatInfoByHeadImageObj(obj)
    ...
    for i = 1, #seatInfoList do
        ...
        if (seatHolder and seatHolder.imagePlayerHead.gameObject == obj) then
            return seatInfo
        end
    end
    return nil
