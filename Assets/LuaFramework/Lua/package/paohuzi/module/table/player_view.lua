...
function PlayerView:initialize(view, index, cloneRoot, module)
    ...
    self.holder = {
        ...
        TingPai = Manager.FindObject(view, "TingPai"),
...
-- 刷新自己的 听牌
function PlayerView:refreshTingPai()
    --  计算位置
    if self.localSeatID == 1 then
        -- XiaZhangHolder 的最后一个    childCount
        local tranTingPai = self.holder.TingPai
        local datat = DataPaoHuZi.Msg_Table_GameStateNTF
        if datat.yi_ting and #datat.yi_ting > 0 then
            tranTingPai.gameObject:SetActive(true)

            for i=1,13 do
                local go =  tranTingPai.transform:Find("PinZhang"):GetChild(i).gameObject
                self.TingPaiMen = self.TingPaiMen or {}
                self.TingPaiMen[i] = self.TingPaiMen[i] or go
                self.TingPaiMen[i]:SetActive(false)
            end
            for i=1,#datat.yi_ting do
                local go = self.TingPaiMen[i]
                go:SetActive(true)
                TableUtilPaoHuZi.set_card(go, datat.yi_ting[i], nil, "ZiPai_PlayCards")
            end
            
        else
            tranTingPai.gameObject:SetActive(false)
        end
